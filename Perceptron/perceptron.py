...
class Perceptron:
    ...
    """
    def fit(self, X, y, n_iter=100,idx=0):
        ...
        if idx !=0:
            ...
            w[0] = 1.0
            b = -np.dot(w, x0)
            self.weights = np.concatenate([w, np.array([b])])
        else: # zero weights initially
    ...
    def MSE(self, X, y):
            """
            True mean squared error between raw scores and labels.
            Assumes y is coded as -1 and 1.
            """
            n_samples = X.shape[0]
            # Add column of 1s (bias)
            X = np.concatenate([X, np.ones((n_samples, 1))], axis=1)
            scores = np.matmul(X, self.weights)      # raw continuous outputs
            return np.mean((y - scores) ** 2)
