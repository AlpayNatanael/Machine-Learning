...
class Perceptron(object):
    ...
    # constractor
    def __init__(self, learning_rate=0.01, n_epoch=100, random_state=1):
    ...
    def fit(self, X, D):
        """ Train the perceptron using LMS.
        ...
        for _ in range(self.n_epoch):
            ...
            for x, d in zip(X, D):
                ...
                update = self.learning_rate * (d - self.predict(x))
    ...
    def predict(self, X):
        ...
        return np.where(self.net_input(X) >= 0, 1, -1)# possible last entry is -1
    ...
    def plot_data(self):#,inputs,targets,weights):
        ...
        for input,d in zip(inputs,targets):
            plt.plot(input[0],input[1],'ro' if (d == 1.0) else 'bo')
